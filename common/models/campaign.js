...
module.exports = function (Campaign) {
  ...
  var beforeRemoteForPermision = function (ctx, modelInstance, next) {
    Campaign.app.models.partner.isAdmin(ctx.req.accessToken, {
      getCampaignIds: true
  ...
  Campaign.costCampaign = function (id, req, res, cb) {
    Campaign.app.models.partner.isAdmin(req.accessToken, {
      getCampaignIds: true
    }, function (err, isAdmin, ids) {
      if (err)
        return cb(err);
      ...
      Campaign.findById(id, function (err, campaign) {
        ...
        Campaign.app.models.click.count({
          campaign_id: id
        }, function (err, countClick) {
          if (err)
            return cb(err);
          Campaign.app.models.impression.count({
            campaign_id: id
          }, function (err, countImp) {
            if (err)
              return cb(err);
            return res.json({
  ...
  Campaign.overAllStates = function (req, campaignId, locationId, startDate, endDate, res, cb) {
    Campaign.app.models.partner.isAdmin(req.accessToken, {
      ...
      if (startDate) where.creation_date = {
        gte: startDate
      };
      if (endDate) where.creation_date = {
        lte: endDate
      };
      ...
      Campaign.findById(campaignId, function (err, campaign) {
        ...
        if (!campaign)
          return cb(ERROR(404, 'campaign not found'));
        Campaign.app.models.click.find({
          where: where,
          fields: {
            client_id: true
          }
        }, function (err, allClicks) {
          if (err)
            return cb(err);
          Campaign.app.models.impression.find({
            where: where,
            fields: {
              client_id: true
            }
          }, function (err, allImpressions) {
            if (err)
              return cb(err);
            var uniqeUsers = {};
            _.each(allClicks, (c) => {
              uniqeUsers[c.client_id] = true;
            });
            _.each(allImpressions, (c) => {
              uniqeUsers[c.client_id] = true;
            });
