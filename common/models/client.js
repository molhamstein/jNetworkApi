...
module.exports = function(Client) {
  ...
  Client.confirmSMS = function(mobile, code, callback) {
    ...
    Client.customSms = function(mobile,message,cb){
		...
		http.get(
			'http://services.mtn.com.sy/General/MTNSERVICES/ConcatenatedSender.aspx?User=LEMA%20ISP%202013&Pass=L1E2M3A4&From=LEMA-ISP&Gsm='+mobile.substring(2)+'&Msg='+String(message)+'&Lang=0&Flash=0',
		function(res) {
			res.on('data', function(data) {
				console.log(data.toString())
				if(data != mobile.substring(2))
					return cb(new Error('mobileWrong'));
                                   //console.log(data.toString());
			  return cb(null,'Done');
			});
		  }
