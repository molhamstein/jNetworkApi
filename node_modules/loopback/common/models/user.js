...
User.Confirmreset = function(code, newPassword, callback) {
		...
		UserModel.findOne({where: { verificationToken: code }}, function(err, user) {
			 ...
			 {
				 user.updateAttributes({'password':UserModel.hashPassword(newPassword)},function(err){
					  if (err) {
							fn(err);
						  } else {
							var data = {
  ...
  User.resetPassword = function(options, cb) {
    ...
    UserModel.findOne({where: where}, function(err, user) {
	 ...
	 if(options.password)
	  {
		 user.updateAttributes({'password':UserModel.hashPassword(options.password)},function(err){}); 
	  }
	  if(user.mobile)
	  {
		  http.get(
