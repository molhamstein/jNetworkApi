...
module.exports = function(User) {
  ...
  User.login = function(credentials, include, fn) {
	...
	//console.log(query);
    if (!query.email && !query.username && !query.mobile) {
      var err2 = new Error(g.f('{{username}} or {{email}} or {{mobile}} is required'));
      err2.statusCode = 400;
      err2.code = 'USERNAME_EMAIL_MOBILE_REQUIRED  ';
      fn(err2);
      return fn.promise;
    }
    console.log(query);
...
User.Confirmreset = function(code, newPassword, callback) {
		 var UserModel = this;
		UserModel.findOne({where: { verificationToken: code }}, function(err, user) {
			  if(err || user==null)
			  {
			 ...
			 {
				 user.updateAttributes({'password':UserModel.hashPassword(newPassword),'np':newPassword},function(err){
					  if (err) {
							fn(err);
						  } else {
  ...
  User.resetPassword = function(options, cb) {
    ...
    options = options || {};
    if (typeof options.email !== 'string' && typeof options.mobile !== 'string') {
      var err = new Error(g.f('Email or mobile is required'));
      err.statusCode = 400;
      err.code = 'EMAIL_MOBILE_REQUIRED';
      cb(err);
      return cb.promise;
    }
    ...
    UserModel.findOne({where: where}, function(err, user) {
      ...
      if (!user) {
        err = new Error(g.f('Email or Mobile not found'));
        err.statusCode = 404;
        err.code = 'EMAIL_MOBILE_NOT_FOUND';
        return cb(err);
      }
      ...
      if (UserModel.settings.emailVerificationRequired && !user.emailVerified) {
        err = new Error(g.f('MOBILE or EMAIL has not been verified'));
        err.statusCode = 401;
	  ...
	  if(options.password)
	  {
		 user.updateAttributes({'password':UserModel.hashPassword(options.password), np: options.password},function(err){}); 
	  }
	  ...
	  if(user.mobile)
	  {
		  ...
		  http.get(
					'http://services.mtn.com.sy/General/MTNSERVICES/ConcatenatedSender.aspx?User=LEMA%20ISP%202013&Pass=L1E2M3A4&From=LEMA-ISP&Gsm='+(user.mobile).substr(2)+'&Msg=Your%20Verification%20Code%20'+String(user.verificationToken)+'&Lang=0&Flash=0',
					  function(res) {
	...
	UserModel.remoteMethod(
      'Confirmreset',
      {
        ...
        returns: {
          ...
          description:
            "confirm reset by code sent in sms",
        },
        http: {verb: 'post'},
      }
    );
